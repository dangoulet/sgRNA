...
###Importing Data and Barcode Files###
```{r}
...
Pool <- readRDS(paste0(Resources, "Corrected Annotations for Complete SKY Library.Rds"))

## Filter Annotation File for Experimental Pools ##
Pool1 <- Pool %>% dplyr::filter(Chip == "CP1406" | Chip == "CP1407")
Pool2 <- Pool %>% dplyr::filter(Chip == "CP1408" | Chip == "CP1409")
...
Pool1_Targets <- long_raw %>% dplyr::filter(Barcode %in% Pool1$Barcode.Sequence) %>% group_by(Sample) %>% summarise(Mean = mean(Counts), Total = sum(Counts), Coverage = sum(Counts)/4093)

Pool2_Targets <- long_raw %>% dplyr::filter(Barcode %in% Pool2$Barcode.Sequence) %>% group_by(Sample) %>% summarise(Mean = mean(Counts), Total = sum(Counts), Coverage = sum(Counts)/4094)
...
log2RPM_Pool1 <- log2RPM_Pool1[,c(1:4,6,12)]
colnames(log2RPM_Pool1) <- c("Barcode", "ID_Barcode","Sample", "Counts", "log2RPM", "Input")
norm_Pool1 <- log2RPM_Pool1 %>% mutate(Norm = log2RPM - Input)

log2RPM_Pool2 <- left_join(long_norm2, Pool2_avg_input, by = "Barcode")
log2RPM_Pool2 <- log2RPM_Pool2[,c(1:4,6,12)]
colnames(log2RPM_Pool2) <- c("Barcode", "ID_Barcode","Sample", "Counts", "log2RPM", "Input")
...
p <- norm_Pool1 %>% dplyr::filter(Sample == "Pool_5_6_hCD19_11_CP1406_CP1407") %>% 
  ggplot(aes(x=Norm))+
...
```{r}
## Annotate Barcodes ##
ann_norm1 <- left_join(norm_Pool1, Pool, by = c("Barcode" = "Barcode"))
ann_norm2 <- left_join(norm_Pool2, Pool, by = c("Barcode" = "Barcode"))
...
## Exclude EGFRv3 Mouse 2 and CD19 Mouse 14 and Mouse 16 from Pool 1 Analysis ##
...
## Exclude EGFRv3 Mouse 3 and 5, CD19 Mouse 12 and 13 from Pool 2 Analysis ##
...
ggsave(here("Pool CP1406 CP1407 Hits Volcano Plot.pdf"),
       device="pdf",
       units="in",
       height=5,
       width=5)
...
ggplot() +
    ...
    geom_point(data=sigZ1, aes(x=reorder(Gene, -zscore), y=zscore), shape=16, size=0.75, color="#FF00FF") +
    geom_text_repel(data = sigZ1, aes(x=reorder(Gene, -zscore), y=zscore), size = 1.75, label = sigZ1$Gene, force = 50, force_pull = 4, direction = "both", max.overlaps = Inf, box.padding = .05, segment.size = 0.2, min.segment.length = .2, max.time = 10) +
    labs(x= "Gene Rank", y = "Z-score", title = "Pool 1 Gene Z-Scores") +
...
ggsave(here("Pool CP1406 and CP1407 Z-Score Rank Plot.pdf"),
       device="pdf",
       units="in",
       height=5,
       width=5)
...
sigRes2 <- L2FC_wide2 %>% dplyr::filter(log10pval > 2.5)
...
Pool2_sig_ess <- Pool2_Essentials %>% dplyr::filter(log10pval > 2 &  L2FC < -1 | L2FC > 1)
...
ggplot() +
  geom_point(data = L2FC_wide2, aes(L2FC, log10pval), shape=16, size=.75, color="#D3D3D3") +
  geom_point(data = sigRes2, aes(L2FC, log10pval), shape = 16, size = 1, color = "#FF00FF") + 
  geom_text_repel(data = sigRes2, aes(L2FC, log10pval), size = 1.75, label = sigRes2$Gene, force = 10, force_pull = .1, direction = "both", max.overlaps = Inf, box.padding = .1, segment.size = .2, min.segment.length = .2) +
  scale_x_continuous(limits = c(-4,4), breaks = seq(-4,4,1)) +
  scale_y_continuous(limits = c(0,6), breaks = seq(0,6,1)) +
...
ggplot() +
    ...
    labs(x= "Gene Rank", y = "Z-score", title = "Pool B Gene Z-Scores") +
    scale_x_discrete(expand=expansion(add=20)) +
    geom_hline(yintercept=2, linetype= "dashed", size=0.25) +
    geom_hline(yintercept=-2, linetype="dashed", size=0.25) +
    theme(legend.position="none",
...
ggsave(here("Pool CP1408 and CP1409 Significant Z-Score Rank Plot All Samples.pdf"),
       device="pdf",
       units="in",
       height=5,
       width=5)
